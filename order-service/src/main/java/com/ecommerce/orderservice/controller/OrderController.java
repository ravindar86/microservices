...
public class OrderController {
    ...
    public String placeOrder(@RequestBody OrderDto orderDto){
        ...
        Supplier<Boolean> booleanSupplier = () -> orderDto.getOrderLineItemsList().stream()
                .allMatch(orderLineItems -> {
                    log.info("Making call to Inventory Service for ProductCode -> {}",orderLineItems.getProductCode());
                    Boolean result = inventoryClient.checkInventory(orderLineItems.getProductCode());

                    return result;
                });
        boolean isStockAvailable = circuitBreaker.run(booleanSupplier,throwable -> handleErrorCase(throwable));


       /* boolean isStockAvailable = orderDto.getOrderLineItemsList().stream()
                                .allMatch(orderLineItems -> inventoryClient.checkInventory(orderLineItems.getProductCode()));
        */
    ...
    private Boolean handleErrorCase(Throwable e) {
